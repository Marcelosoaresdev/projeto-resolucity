...
function createReportRepository(dbQuery = query) {
    return {
        ...
        async createReport(userId, categoria, tipo, endereco, descricao, latitude, longitude) {
            ...
            const updated = await dbQuery(
                'UPDATE reports SET protocolo = $1 WHERE id = $2 RETURNING *',
                [protocolo, savedReport.id]
            );
        ...
        async getStats(period, startDate, endDate) {
            ...
            reports
                ...
                .forEach(r => {
                    resolvidosPorCategoria[r.categoria] =
                        (resolvidosPorCategoria[r.categoria] || 0) + 1;
                });
            ...
            reports.forEach(r => {
                ...
                if (
                    r.status === 'resolvido' &&
                    resolvidosPorMes[key] !== undefined
                ) {
...
function extractBairro(endereco) {
    ...
    // Padrão 1:
    // "Rua X, NUMERO - BAIRRO, Cidade - Estado"
    const match1 = endereco.match(
        /,\s*([^,]+?)\s*-\s*[^,]+$/
    );
