...
function createUserRepository(dbQuery = query) {
    return {
        ...
        async findByConfirmationToken(token) {
            const { rows } = await dbQuery(
                'SELECT * FROM users WHERE "confirmationToken" = $1 LIMIT 1',
                [token]
            );
        ...
        async activateUser(token) {
            ...
            if (user.status === 'ativo') {
                return { error: 'Link já utilizado. Faça login normalmente.' };
            }

            if (new Date(user.confirmationExpiresAt) < new Date()) {
                return { error: 'Link expirado. Solicite um novo.' };
            }

            const result = await dbQuery(`
                UPDATE users
                SET status = 'ativo', "confirmationToken" = NULL, "confirmationExpiresAt" = NULL
                WHERE id = $1
                RETURNING *
            `, [user.id]);
        ...
        async updateUser(id, data) {
            try {
                const { rows } = await dbQuery(`
                    UPDATE users
                    SET nome = $1, cpf = $2, nascimento = $3, telefone = $4, email = $5
                    WHERE id = $6
                    RETURNING *
                `, [
                    data.nome,
